...
- **Gamma 前導頁提示詞生成**：生成適用於 Gamma.app 的專業提示詞，快速建立互動式前導頁

### 🎨 商品海報提案與圖片生成

- **三個風格海報提案**：AI 自動生成三個不同風格的商品海報提案，包含設計概念、色彩方案、視覺元素
- **圖片生成提示詞**：每個提案都包含詳細的圖片生成提示詞，可直接用於圖片生成工具
- **真實圖片生成**：整合 Google Gemini Imagen API，支援直接生成海報圖片
- **多種尺寸選擇**：支援正方形、直式、手機直式、橫式等多種尺寸
- **參考圖片支援**：可上傳參考圖片，AI 會分析其風格並應用於生成的海報
